...
module.exports = {
  ...
  topTenInAllDay: () => {
    return db.load(`SELECT tb1.*, ac.name FROM (SELECT DISTINCT teacherId, SUM(price) as price FROM contract WHERE state = 3 GROUP by teacherId LIMIT 5) as tb1 JOIN account as ac on tb1.teacherId = ac.userId order by price `)
  },
